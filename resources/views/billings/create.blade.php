<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('billings.store') }}" method="POST" class="space-y-6">
                @csrf
                        ...
                        <div>
                            ...
                            <select id="funeral_service_id" name="funeral_service_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                @foreach($funeralServices as $service)
                                    <option value="{{ $service->id }}" @selected(old('funeral_service_id') == $service->id)>
                ...
                <div class="bg-white p-6 shadow sm:rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="issued_at" name="issued_at" type="date" class="mt-1 block w-full" value="{{ old('issued_at') }}" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <x-text-input id="subtotal" name="subtotal" type="number" step="0.01" class="mt-1 block w-full" value="{{ old('subtotal') }}" />
                        </div>
                        <div>
                            <x-input-label for="tax_rate" :value="gettext('VAT (%)')" />
                            <x-text-input id="tax_rate" name="tax_rate" type="number" step="0.01" class="mt-1 block w-full" value="{{ old('tax_rate') }}" />
                        </div>
                        <div class="md:col-span-2">
                            <x-input-label for="notes" :value="gettext('Notes')" />
                            <textarea id="notes" name="notes" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="4">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </div>
